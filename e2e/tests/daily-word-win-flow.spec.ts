...
import { injectDailyWordState, WIN_STATE } from "./helpers/daily_word";
...
test.describe("Daily Word — win flow", () => {
  test("win modal is visible after loading a won state", async ({ page }) => {
    ...
    await expect(
      page.getByTestId("daily-word-result-title").getByText("You Win!"),
  ...
  test("win modal shows guess count", async ({ page }) => {
    ...
    await expect(page.getByText(/in 3 guesses/i)).toBeVisible({
      timeout: 5_000,
    });
  });
  ...
  test("win modal shows next-word countdown in HH:MM:SS format", async ({
    page,
  }) => {
    await loadWinState(page);
