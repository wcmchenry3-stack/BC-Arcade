...
test("Daily Word — typed letters in current row persist after navigate away and back", async ({
  ...
  await page.goto("/");
  await page
    .getByRole("button", { name: "Play Daily Word" })
...
test("Daily Word — submitted row colors persist after page reload", async ({
  page,
}) => {
...
test("Daily Word — completed win state shows win modal after reload", async ({
  page,
}) => {
