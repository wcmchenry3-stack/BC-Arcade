...
function selectCardToPlayHard(
  ...
  // conserve high cards for future use rather than exhausting them here (#1626).
  if (!isMoonAttempt) {
    const first = trick[0];
    if (first && trickPoints(trick) >= 3) {
      const inSuit = valid.filter((c) => c.suit === first.card.suit);
      if (inSuit.length > 0) {
        let winningRank = 0;
        for (const tc of trick) {
          if (tc.card.suit === first.card.suit && aceHigh(tc.card.rank) > winningRank) {
            winningRank = aceHigh(tc.card.rank);
          }
        }
        ...
        if (losing.length > 0) {
          ...
          if (qSpade) return qSpade; // always shed Q♠ first
          return lowest(losing) ?? valid[0]!;
        }
      }
    }
  }
