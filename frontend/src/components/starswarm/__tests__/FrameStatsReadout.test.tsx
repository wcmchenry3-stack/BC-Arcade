...
describe("FrameStatsReadout", () => {
  ...
  it("shows the renderer and the latest summary, refreshed on its own timer", async () => {
    let current: FrameStatsSummary | null = null;
    const read = jest.fn(() => current);
    await render(<FrameStatsReadout read={read} renderer="picture" />);
    expect(screen.getByText("picture · frame: no samples")).toBeTruthy();

    current = { avgMs: 16.7, p95Ms: 17.9, frames: 60, commitsPerSec: 0 };
    await act(async () => {
      jest.advanceTimersByTime(FRAME_STATS_POLL_MS);
    });
    expect(screen.getByText("picture · 16.7 ms avg · 17.9 p95 · 60 f · 0 commits/s")).toBeTruthy();
  });

  it("stops polling when unmounted", async () => {
    const read = jest.fn(() => null);
    const { unmount } = await render(<FrameStatsReadout read={read} renderer="legacy" />);
    await unmount();
