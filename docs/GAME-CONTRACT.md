...
- [ ] **Stats menu item** (#2635) — pass `gameType="<game>"` to every `GameShell` the game renders (a required prop): its ⋯ menu then gets a "Stats" item that opens the shared `GameStats` screen (`/stats/me` for that game). `gameType={null}` is only for screens that are not one game's play screen (a live scorecard, a run history, a dev tool)
