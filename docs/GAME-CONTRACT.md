...
- **Not yet sent by the client:** Sort sends `level`/`moves` rather than `level_reached`/`total_moves`; its Phase 2 story (#2625) makes the client send the declared keys. FreeCell's win sends `final_score = moveCount` since #2632 (installed builds still send none).
