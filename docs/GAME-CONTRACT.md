...
| `partition_values`     | `tuple[tuple[str, tuple[str, ...]], ...]` | `(key, allowed values)`: the only values that key has a board for; a request for any other is a 400. The metadata and result models must accept exactly these values (`test_partition_values_are_exactly_what_the_models_accept`). A key not listed takes any value. Keys must be in `partitions`, once each; values non-empty and distinct; that key's `partition_defaults` and `partition_max_values` values must be among them. Read with `allowed_values(key)` / `is_allowed(key, value)`. |
...
| Starswarm   | `StarSwarmMetadata` | `difficulty_tier: DifficultyTier \| None`, one of the app's ten tiers (`DIFFICULTY_TIERS`). The router's own leaderboard rows (`POST /starswarm/score`) are written directly and hold `player_name` |
