...
- **Sort** (#2625): every solved level is a session row with `level`, `moves` and `undos` (`SortResult`). Only the first solve of the player's frontier level is scored: `final_score` and `level_reached` are the level, and `total_moves` is the sum of the player's best moves over levels 1 to it (`@sort/best_moves`; left out when a lower level has no best on record, so the row ranks after equal levels that have one). Replays and abandons carry neither key and never rank.
