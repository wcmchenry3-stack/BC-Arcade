...
- **`GET /games/{id}/rank`** (#2677) is the result card's call: where this game puts the caller, without setting anything. It returns `{rank, is_best, ranked, reason}`. When `ranked` is true, `rank` and `is_best` are exactly what `PATCH /games/{id}/name` would return. When it is false, both are null and `reason` is the first of these that applies: `board_disabled` (the board has `enabled=False`, e.g. Blackjack), `not_rankable` (the game can't rank, for any of the name route's 400 reasons: unfinished, no metric value, abandoned, a non-qualifying outcome, a disallowed partition value, a bad or over-cap metric), or `no_name` (the player has no display name, so no board shows them and no rank is computed). `not_rankable` comes before `no_name`, so a card never asks for a name the game couldn't use. 403 if another session owns the game or a premium game isn't entitled, 404 if the game doesn't exist or its game has no board definition, 400 without `X-Session-ID`. It is read-only: no name and no metadata are written. Rate limit: 60/minute per session with a 300/minute per-IP backstop, like the leaderboard.
...
- **Result-card flow for session boards** (#2677): the card doesn't submit a score. A game passes `sessionBoardAdapter(gameType)` (`frontend/src/game/_shared/sessionBoardAdapter.ts`) to `useLeaderboardSubmit` and calls `submit({ gameId })` when the game ends. The adapter flushes the local game queue and any pending display-name sync, then calls `gamesApi.getRank(gameId)` (`frontend/src/api/games.ts`), retrying a 404 or `not_rankable` briefly while the completion lands (`retryUntilGameSynced`). With no local name the card shows `needsName`, and `provideName()` saves it (`saveDisplayName`, synced through `PUT /players/me`) and then fetches the rank; a server `no_name` also shows the prompt once the name sync has settled. Online with a rank, the card shows `saved` and the rank when it is top 10. Offline, the card shows `offline` and **nothing is queued**: the game syncs through `SyncWorker` and the name through `displayNameSync`, and the card fetches the rank again on reconnect if it is still mounted. A game that can't rank shows `saved` with no rank; other failures show `error` with Retry. Legacy per-game adapters (`LeaderboardAdapter`, with a `scoreQueue` payload) keep working until #2625–#2632 move each game over.
