...
- **Duration:** Cascade's own `duration_ms`: wall-clock time since the session opened (`Date.now() - gameStartTimeRef`). Time while another screen covers the board is taken out (the loop stops and `gameStartTimeRef` moves forward on `focus`, #2743). Because the value is > 0 it wins over `useGameSync`'s active-play window, so backgrounded time is still counted. A game continued after an app kill counts from the relaunch, so the time before the kill is lost (#2750).
...
- #2750: backgrounded time is counted in the reported play time (see [Scoring](#scoring-persistence), Duration)
