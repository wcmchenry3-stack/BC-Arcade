...
- Endpoints: `backend/starswarm/router.py` — legacy. `POST /starswarm/score` stays for installed builds; the app no longer calls it (#2626). `GET /starswarm/leaderboard` stays too; the app stopped reading it when the Ranks tab was retired (#2634) and opens Star Swarm's board through the shared `LeaderboardScreen` (#2633)
