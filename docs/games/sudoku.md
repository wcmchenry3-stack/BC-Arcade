...
- **Duration:** Sudoku's own timer, from the first input, with the time the player is away taken out: the app in the background or another screen covering the puzzle (`usePauseWhileAway` in `SudokuScreen.tsx`); a saved puzzle that loads while the player is away starts paused. It wins over `useGameSync`'s window. The elapsed time is not saved: a relaunch restarts it from 0 (intentional, per the comment in `SudokuScreen.tsx`), so time played before an app kill is not counted.
