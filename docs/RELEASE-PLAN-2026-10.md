...
| 1918/2277 | Keystore rotation + secret removal | **#2414 merged Sep 19** (passwords out of the repo, gitleaks repaired). **Still open: the owner's rotation steps** — as of the merge there was no user-level `~/.gradle/gradle.properties` on the PC, so a local release build is debug-signed (Play rejects it) until steps 1–5 are done. GitHub auto-closed #1918/#2277 on merge — treat the rotation as not finished until `:app:signingReport` shows the upload key's SHA-1 for the `release` variant. PC + Play Console; Sat 19 — procedure in the schedule row below |
...
| **Tue 22** | XP: schemas/router wiring + backend tests green. | **Cut TestFlight internal build #1** (hide-premium in): confirm premium tiles absent in a real release build. **Also confirm the build shows version 1.0.9** (first Xcode Cloud build after #2412 — opens a new TestFlight version train; earlier uploads were 1.0.0). |
