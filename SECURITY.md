...
_Not every check applies to every project — mobile-only repos skip frontend npm audit; static sites skip Python checks. All checks are listed here so the full posture is visible even when a given check is not yet wired up._
